...
int playroom_init(struct playroom *room)
{
	...
	rv &= pthread_cond_init(&room->dogs_and_birds_out, NULL);
	if (rv)
		return rv;
...
void cat_enter(struct playroom *room)
{
	...
	while (room->dogs.n > 0 || room->birds.n > 0) {
		...
		room->cats.wait++;
		pthread_cond_wait(&room->dogs_and_birds_out, &room->lock);
		room->cats.wait--;
		waits++;
	}
	room->cats.total++;
...
void dog_exit(struct playroom *room)
{
	pthread_mutex_lock(&room->lock);
	room->dogs.n--;
	if (room->dogs.n <= 0 && room->birds.n <= 0) {
		pthread_cond_broadcast(&room->dogs_and_birds_out);
	}
	pthread_mutex_unlock(&room->lock);
}
...
void bird_exit(struct playroom *room)
{
	pthread_mutex_lock(&room->lock);
	room->birds.n--;
	if (room->birds.n <= 0 && room->dogs.n <= 0) {
		pthread_cond_broadcast(&room->dogs_and_birds_out);
	}
	pthread_mutex_unlock(&room->lock);
}
...
void playroom_print(struct playroom *room)
{
	...
	pthread_mutex_lock(&room->lock);
	printf("play: %2dc %2dd %2db     wait: %2dc %2dd %2db     total: %2dc %2dd %2db     useless wakeups: %2dc %2dd %2db\n",
			...
			room->cats.wakeup, room->dogs.wakeup, room->birds.wakeup);
	pthread_mutex_unlock(&room->lock);
}
